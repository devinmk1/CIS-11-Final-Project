...
; MAIN FUNCTION TO CALCULATE MIN
CALC_MIN
	LD R1, NUM_GRADES		; LOAD NUMBER OF GRADES 
	LEA R2, GRADES			; LOAD ARRAY OF GRADES
	LD R4, GRADES			; R4 = TO FIRST NUMBER IN ARRAY
...
PUSH 	ST R7, SAVELOC2			; STORE JSR LOCATION
	...
	LD R4, SP			; SP = x4000
	ADD R4, R4, #0			; COMPARE WITH STACK POINTER
	BRnz ERROR_STACK		; CHECK TO SEE IF STACK IS EMPTY
	ADD R4, R4, #-1			; DECREMENT POINTER
	STR R0, R4, #0			; STORE NUMBER IN R0 TO STACK
	ST R4, SP			; SAVE POINTER LOCATION
	LD R7, SAVELOC2			; RESTORE LOCATION
	RET				; RETURN SUBROUTINE

SP	.FILL	x4000			; SP = STACK POINTER

POP	AND R4, R4, #0			; CLEAR R4
	AND R1, R1, #0			; CLEAR R1
	LD R4, SP			; LOAD POINTER
	ST R1, SAVELOC3			; STORE JSR LOCATION
	LD R1, EMPTY
	ADD R1, R1, R4			; COMPARE STACK POINTER
	BRp ERROR_STACK			; SEE IF STACK IS EMPTY
	LD R1, SAVELOC3
	LDR R0, R4, #0			; LOAD STACK VALUE INTO R0
	ST R7, SAVELOC4			; STORE JSR LOCATION

	OUT				; PRINT LETTER FROM STACK
	LD R0, SPACE			; ADD A SPACE
	OUT				; PRINT SPACE

	ADD R4, R4, #1			; INCREMENT POINTER
	ST R4, SP			; STORE POINTER LOCATION
	LD R7, SAVELOC4			; LOAD POINTER LOCATION
	RET				

EMPTY	.FILL	xC000			; = -4000
	
ERROR_STACK	LEA R0, ERROR		; LOAD STRING TO R0
		OUT			; OUTPUT
		HALT			; STOPS PROGRAM
